...
// error check name and description
  ...
  const description = alignmentForm.value.description || JSON.stringify({
    ...
    ...alignmentForm.value.settings,
  }).replaceAll(new RegExp('[\{\"\}]', 'g'), '').replaceAll(',', ' ')


  fetch("/api/generateAlignment", {
    method: "POST",
    body: JSON.stringify({...alignmentForm.value, description}),
    headers: {
      ...
      <div class="col mb-4" v-for="a in alignments" :key="a.id">
        ...
        >
          <div
              ...
              @click.prevent="
              togglePush(store.selectedAlignments, { id: a.id, name: a.name, description:a.description })
            "
          >
