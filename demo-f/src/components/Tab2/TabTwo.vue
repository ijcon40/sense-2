...
// returns the embedding id
  ...
  const description = embeddingForm.description || JSON.stringify({
    type: embeddingForm.embeddingType,
    ...embeddingForm.settings,
  }).replaceAll(new RegExp('[\{\"\}]', 'g'), '').replaceAll(',', ' ')
  fetch("/api/generateEmbedding", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify({...embeddingForm, description: description})
